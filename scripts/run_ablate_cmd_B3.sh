#!/bin/bash
# Ablation B, relaunch 3 (head service on CPU, flow at 0.42; right/cfl at t0 0.3 and cfr at 0.7/0.9)
...
OUT=$RUN/ablate_cmd_B3_scores.txt
rm -f $RUN/ablate_cmd_B3.done $OUT
...
setsid $EV $PINENV JAX_PLATFORMS=cpu CUDA_VISIBLE_DEVICES=-1 \
...
curl -s http://127.0.0.1:$HPORT/ >/dev/null || { echo "HEAD_TIMEOUT" >> $OUT; echo DONE > $RUN/ablate_cmd_B3.done; exit 1; }
for CT in right:0.3 cfl:0.3 cfr:0.7 cfr:0.9; do
  C=${CT%:*}; T0=${CT#*:}; TAG=sdehead_t${T0/./}_$C; killport $PORT
  setsid $EV SNMVP_ZERO_PAD_ACTIONS=1 XLA_PYTHON_CLIENT_PREALLOCATE=true XLA_PYTHON_CLIENT_MEM_FRACTION=0.42 CUDA_VISIBLE_DEVICES=0 \
...
killport $HPORT
echo DONE > $RUN/ablate_cmd_B3.done
