#!/bin/bash
# Attribute Table 1's gain (2026-09-03, Denis: "try that"):
#  A. xswap served with PLAIN noise (pin off at inference, sigma_serve = 1.5) on the four atomics.
#  B. SDEdit on the UNPINNED scratch3 guided by OUR head's decoded command U c (head-only service on
#     the xswap checkpoint): CFR at t0 0.3 / 0.5, then left / right / CFL at t0 0.3.
set -u
...
OUT=$RUN/ablate_cmd_scores.txt
rm -f $RUN/ablate_cmd.done $OUT
...
# ---- A: pin off
for C in cfr left right cfl; do
  TAG=pinoff_$C; killport $PORT
  setsid $EV $PINENV SNMVP_PIN_OFF=1 SNMVP_PIN_OFF_SIGMA=1.5 CLOG=$RUN/clog_${TAG}.npy $GPU \
    $VENVPY $RD/serve_gate_pin_joint.py --ckpt $CK_PIN --config pi0_gate --norm $HFB/assets/gate_nav --pin-u $U --port $PORT \
...
# ---- B: head service (xswap) + SDEdit flow (scratch3)
setsid $EV $PINENV $GPU $VENVPY $RD/serve_head_only.py --ckpt $CK_PIN --norm $HFB/assets/gate_nav --port $HPORT \
  >> $RUN/sv_head_only.log 2>&1 </dev/null & disown
for k in $(seq 1 150); do curl -s http://127.0.0.1:$HPORT/ >/dev/null && break; sleep 3; done
curl -s http://127.0.0.1:$HPORT/ >/dev/null || { echo "HEAD_TIMEOUT" >> $OUT; echo DONE > $RUN/ablate_cmd.done; exit 1; }
for CT in cfr:0.3 cfr:0.5 left:0.3 right:0.3 cfl:0.3; do
  C=${CT%:*}; T0=${CT#*:}; TAG=sdehead_t${T0/./}_$C; killport $PORT
  setsid $EV SNMVP_ZERO_PAD_ACTIONS=1 $GPU $VENVPY $RD/serve_gate_sdedit.py --ckpt $CK_SCR --config pi0_gate \
    --norm $HFB/assets/gate_nav --head-url http://127.0.0.1:$HPORT --pin-u $U --t0 $T0 --port $PORT \
    >> $RUN/sv_${TAG}.log 2>&1 </dev/null & disown
  for k in $(seq 1 150); do ss -ltn | grep -q ":$PORT " && break; sleep 3; done
  ss -ltn | grep -q ":$PORT " || { echo "SERVER_TIMEOUT $TAG" >> $OUT; continue; }
  roll_and_score $TAG $C; killport $PORT
