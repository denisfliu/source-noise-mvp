...
TRAINEXTRA="SNMVP_HEAD_DETACH=0 SNMVP_HEAD_LAM=0.3 SNMVP_HEAD_GMM=1 SNMVP_PIN_NOISE=1.5 SNMVP_PIN_NOISE_RAND=1 SNMVP_PIN_NOISE_COND=1"
CK=/home/dfliu/code/openpi-snmvp/checkpoints/pi0_gate/gate_pin_joint_$NAME/4999
PORT=8996
...
rm -rf /home/dfliu/code/openpi-snmvp/checkpoints/pi0_gate/gate_pin_joint_$NAME/*/train_state
...
roll () {  # claim-tier center cells keep video; OUT template required or frames are discarded
  env CUDA_VISIBLE_DEVICES=0 PORT=$P2 SIDE=$2 SCENE=$3 NCH=$5 APC=50 TRIALS=$6 VIDEO=1 \
    OUT=$RUN/overlay_${NAME}_$1_{t}.mp4 PROMPT="$4" TRAJ=$RUN/traj_${NAME}_$1_{t}.npy \
...
wait
env CUDA_VISIBLE_DEVICES=0 PORT=$P2 SIDE=left SCENE=left_and_center NCH=14 APC=50 TRIALS=5 VIDEO=0 PROMPT="$CMP_L" TRAJ=$RUN/traj_${NAME}_cmpl_{t}.npy $TV $RD/gate_rollout_batch.py > $RUN/roll_${NAME}_cmpl.log 2>&1
env CUDA_VISIBLE_DEVICES=0 PORT=$P2 SIDE=right SCENE=right_and_center NCH=14 APC=50 TRIALS=5 VIDEO=0 \
  PROMPT="$CMP_R" TRAJ=$RUN/traj_${NAME}_cmpr_{t}.npy $TV $RD/gate_rollout_batch.py > $RUN/roll_${NAME}_cmpr.log 2>&1
serve_down $P2
