...
PORT=9062
rm -f $RUN/sketch_min5.done $RUN/traj_skm5_*.npy $RUN/sketch_min5_scores.txt
cd $RD
run_cell() { # tag scene client_prompt
  ...
  sleep 3
  setsid $EV $BASE $HEADENV SNMVP_PIN_PROMPT=$RD/sketch_${TAG}_min5.json \
    CLOG=$RUN/clog_skm5_${TAG}.npy \
    ...
    --port $PORT >> $RUN/sv_skm5_${TAG}.log 2>&1 </dev/null & disown
  for k in $(seq 1 150); do ss -ltn | grep -q ":$PORT " && break; sleep 3; done
  ss -ltn | grep -q ":$PORT " || { echo "SERVER_TIMEOUT $TAG" >> $RUN/sketch_min5.done; return 1; }
  env CUDA_VISIBLE_DEVICES=0 PORT=$PORT SIDE=${SCENE%%_*} SCENE=$SCENE NCH=14 APC=50 TRIALS=5 VIDEO=0 \
    PROMPT="$CPROMPT" TRAJ=$RUN/traj_skm5_${TAG}_{t}.npy $TV $RD/gate_rollout_batch.py \
    > $RUN/roll_skm5_${TAG}.log 2>&1
  for p in $(pgrep -f "$SRV --ckpt .* --port $PORT"); do kill -9 "$p" 2>/dev/null; done
  { echo "== gmsig3 + MINIMAL 5-point sketch, $TAG (route-clean judge)"
    $EV JAX_PLATFORMS=cpu CUDA_VISIBLE_DEVICES=-1 $VENVPY $RD/gate_success.py \
      --traj $RUN/traj_skm5_${TAG}_*.npy --side $SCENE
    $TV $RD/gate_clearance.py --scene $SCENE --traj $RUN/traj_skm5_${TAG}_*.npy
  } >> $RUN/sketch_min5_scores.txt 2>&1
}

run_cell cmpr right_and_center "$CMP_R"
echo DONE >> $RUN/sketch_min5.done
