#!/bin/bash
# Bad sketches on the MIXED-data checkpoints (2026-09-20): gmsig3 pin sigma 0 / 0.5, projection + SDEdit on gate_scratch3.
...
PORT=${PORT:-9160}; OUT=$RUN/badsketch_mixed_scores.txt; rm -f $RUN/badsketch_mixed.done $OUT
cd $RD
...
cell () { # tag mode sketch side scene prompt
  ...
  killport
  { echo "== badsketch mixed: $TAG (mode=$MODE sketch=$SK scene=$SCENE)"
...
cell bsm_pin0_L  pin     sketch_cmpl_min4.json  left  left_and_center  "$CMP_L"
cell bsm_pin05_L pin     sketch_cmpl_min4s.json left  left_and_center  "$CMP_L"
cell bsm_vproj_L vproj   sketch_cmpl_min4.json  left  left_and_center  "$CMP_L"
cell bsm_sde05_L sde:0.5 sketch_cmpl_min4.json  left  left_and_center  "$CMP_L"
cell bsm_pin0_R  pin     sketch_cmpr_min4.json  right right_and_center "$CMP_R"
cell bsm_pin05_R pin     sketch_cmpr_min4s.json right right_and_center "$CMP_R"
cell bsm_vproj_R vproj   sketch_cmpr_min4.json  right right_and_center "$CMP_R"
cell bsm_sde05_R sde:0.5 sketch_cmpr_min4.json  right right_and_center "$CMP_R"
echo DONE > $RUN/badsketch_mixed.done
