...
set -u
TAG=${1:?tag}; SCENE=${2:?scene}; SIDE=${3:?side}; PROMPT=${4:?prompt}; NCH=${5:-10}
...
PORT=${PORT:-9160}; AD=$RUN/agent_sim
...
setsid env CUDA_VISIBLE_DEVICES=0 PORT=$PORT SIDE=$SIDE SCENE=$SCENE NCH=$NCH APC=50 TRIALS=1 VIDEO=1 \
...
echo "flight $TAG started; review with: python experiments/rung3/agent_sim_cli.py wait"
