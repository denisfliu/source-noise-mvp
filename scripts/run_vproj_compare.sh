...
GPU="XLA_PYTHON_CLIENT_PREALLOCATE=true XLA_PYTHON_CLIENT_MEM_FRACTION=0.40 CUDA_VISIBLE_DEVICES=0"
...
PORT=${PORT:-9160}; OUT=$RUN/vproj_scores.txt; rm -f $RUN/vproj.done $OUT
cd $RD
...
for M in ours sde:0.5 inject vproj; do
  T=${M/:/}; T=${T/./}
