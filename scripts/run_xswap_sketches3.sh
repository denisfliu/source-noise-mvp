#!/bin/bash
# Third xswap sketch batch (2026-09-02): the two Table-3 drawings REDRAWN per Denis (4-click L->C point 3
# moved; hand-drawn R->C points 2, 4, 5 moved), both xswap seeds, sigma variants as in the originals.
set -u
...
PORT=9132
OUT=$RUN/xsk3_scores.txt
rm -f $RUN/xsk3.done $OUT
cd $RD
...
for S in 42 s7; do
  ...
  cell ${P}_cmpl_min4v2  $CK $SM sketch_cmpl_min4v2.json  left  left_and_center  "$CMP_L"
  cell ${P}_cmpl_min4sv2 $CK $SM sketch_cmpl_min4sv2.json left  left_and_center  "$CMP_L"
  cell ${P}_cmpr_r2      $CK $SM sketch_cmpr_denis_r2.json right right_and_center "$CMP_R"
done
echo DONE > $RUN/xsk3.done
