...
OUT=$RUN/ablate_cmd_B2_scores.txt
rm -f $RUN/ablate_cmd_B2.done $OUT
...
curl -s http://127.0.0.1:$HPORT/ >/dev/null || { echo "HEAD_TIMEOUT" >> $OUT; echo DONE > $RUN/ablate_cmd_B2.done; exit 1; }
for k in $(seq 1 240); do [ -f $RUN/ablate_cmd_B.done ] && break; sleep 30; done
for CT in cfr:0.7 cfr:0.9; do
...
killport $HPORT
echo DONE > $RUN/ablate_cmd_B2.done
