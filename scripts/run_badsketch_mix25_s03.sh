#!/bin/bash
# Ours (gmsig3) at sigma 0.3 on the flawed center sketch (2026-09-25; Denis: match the figure's sigma to v-proj s 0.3).
# Tag bsm25_pin03_L. Based on run_badsketch_mix25.sh:
...
PORT=${PORT:-9020}; OUT=$RUN/badsketchmix25s03_scores.txt; rm -f $RUN/badsketchmix25s03.done $OUT
cd $RD
...
cell bsm25_pin03_L pin sketch_cmpl_min4s03.json left left_and_center "$CMP_L"
echo DONE > $RUN/badsketchmix25s03.done
