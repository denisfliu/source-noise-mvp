#!/bin/bash
# Center-gate proof of concept on the MIXED-data checkpoints (2026-09-25; Denis: use the gate center as a proof of concept):
...
PORT=${PORT:-9020}; OUT=$RUN/badsketchmix25_scores.txt; rm -f $RUN/badsketchmix25.done $OUT
cd $RD
...
cell bsm25_pin0_L  pin     sketch_cmpl_min4.json  left  left_and_center  "$CMP_L"
cell bsm25_pin05_L pin     sketch_cmpl_min4s.json left  left_and_center  "$CMP_L"
cell bsm25_vproj_L vproj   sketch_cmpl_min4.json  left  left_and_center  "$CMP_L"
cell bsm25_sde05_L sde:0.5 sketch_cmpl_min4.json  left  left_and_center  "$CMP_L"
echo DONE > $RUN/badsketchmix25.done
