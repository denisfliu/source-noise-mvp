#!/bin/bash
# Pinned-SOURCE-SAMPLE diagnostic (2026-09-04): execute z = g - UU^Tg + Uc itself, no denoising.
...
U=$RD/pin_U_mh16.npy
PINENV="SNMVP_HEAD=1 SNMVP_ZERO_PAD_ACTIONS=1 SNMVP_PIN_U=$U SNMVP_HEAD_DETACH=0 SNMVP_HEAD_LAM=0.3 SNMVP_HEAD_GMM=1 SNMVP_PIN_NOISE=1.5 SNMVP_PIN_NOISE_RAND=1 SNMVP_PIN_NOISE_COND=1 SNMVP_SIGMA_MAP=$RD/sigma_map_xswap.json SNMVP_PIN_DECODE_ONLY=2"
CK=/home/dfliu/code/openpi-snmvp/checkpoints/pi0_gate3/gate_pin_joint_xswap/4999
PORT=9221
OUT=$RUN/decode_src_scores.txt
rm -f $RUN/decode_src.done $OUT
...
cell src_cfr  ""  right center center_from_right "go through the center gate from the right and hover over the stuffed animal"
cell src_cmpl "SNMVP_PIN_PROMPT=$RD/sketch_cmpl_denis.json" left left_and_center left_and_center "$CMP_L"
echo DONE > $RUN/decode_src.done
