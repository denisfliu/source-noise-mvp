...
#
#   run_sixcell_plain.sh NAME CKPT PORT TRIALS   (scratch arms: plain pi0 serve,
#   no pin/head — serving pin-free flows through the pin server is a known artifact source)
#
# Markers/outputs: ev6_$NAME.done, ev6_${NAME}_scores.txt (L/R), ev6_${NAME}_ctr_scores.txt.
set -u
NAME=$1; CK=$2; PORT=$3; TRIALS=$4
GPU=0
...
serve_up () {  # port clog-suffix
  ...
  sleep 3
  setsid $EV XLA_PYTHON_CLIENT_PREALLOCATE=true \
    XLA_PYTHON_CLIENT_MEM_FRACTION=0.45 CUDA_VISIBLE_DEVICES=$GPU $VENVPY $RD/$SRV \
    --ckpt $CK --config pi0_gate --norm $HFB/assets/gate_nav --port $1 \
