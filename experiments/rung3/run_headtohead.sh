...
# pin's state->c prior; closed-loop eval pin (prior) vs scratch (unpinned). All -> htoh.status.
set -uo pipefail
...
for g in 11 21; do
  ...
  # adapt both arms
  train "hh_pin_t${g}"     "$CKD/snmvp_src_pin_rrr/4999"       "$U" "$FS"; echo "ADAPT pin g=$g rc=$? $(date -u +%H:%M:%S)" >> "$ST"
  train "hh_scratch_t${g}" "$CKD/snmvp_src_scratch_shared/4999" ""   "$FS"; echo "ADAPT scratch g=$g rc=$? $(date -u +%H:%M:%S)" >> "$ST"
  # pin prior on the same demos
  SNMVP_PIN_U=$U SNMVP_EPISODES=$FS SNMVP_PRIOR_OUT=$RD/prior_hh_t${g}.npz SNMVP_NB=40 \
    CUDA_VISIBLE_DEVICES=$GPU "$HOME/code/openpi/.venv/bin/python" "$RD/fit_prior_hh.py" >> "$ST" 2>&1
  # closed-loop
  echo "EVAL pin g=$g" >> "$ST";     serve_eval "hh_pin_t${g}"     "$RD/prior_hh_t${g}.npz" "$su" "$LOCAL" "$RD/hh_pin_t${g}.json"
  echo "EVAL scratch g=$g" >> "$ST"; serve_eval "hh_scratch_t${g}" ""                        "$su" "$LOCAL" "$RD/hh_scratch_t${g}.json"
done
