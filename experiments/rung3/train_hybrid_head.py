"""HYBRID stage-1 head: p(c | [model_state, phi]) — state supplies the off-manifold
RESTORING FIELD the pure-feature heads lack (cmd_compare 2026-08-06: feature heads go
flat once OOD while state heads command correction); phi supplies language/task. Still
NO one-hot. Otherwise identical to train_vlmflow_head.py.
...
X = np.concatenate([np.stack(MS), X], 1).astype(np.float32)  # [state | phi]
...
print("(baselines: vlm-only flow 0.911, mlp_rend 0.926, ridge_rend 0.809 on these rows)", flush=True)
torch.save({"state_dict": net.state_dict(), "xmu": xmu.astype(np.float32),
            ...
            "K": 5, "arch": "cfm-3x512-hybrid"}, os.path.join(RD, "hybrid_head_rend.pt"))
print("saved hybrid_head_rend.pt"); print("HYBRID_DONE", flush=True)
