"""Two-stage flow, VLA-native: stage 1 = CFM head p(c | phi) on the fused VLM
prefix feature of the CURRENT obs (no one-hot, no state input — language enters
through the prompt tokens); stage 2 = the pi0 flow with c pinned in the source
noise. One sample per chunk, Euler-10."""
...
class VlmFlowPin:
    def __init__(self, policy, pin_u, head_path, steps=10, k=8):
        ...
        self.H, self.AD = d["H"], d["AD"]; self.steps = steps; self.k = k
        self._rng = np.random.default_rng()
        self._ema = None; self._prev_pos = None
        print(f"[vlmflow] two-stage: CFM(phi)->c->pin, steps={steps} k-mean={k} ema=0.5", flush=True)

    def infer(self, obs):
        phi = gc.ctx_pool(self.policy, [obs])[0]
        xn = torch.tensor(((phi - self.xmu) / self.xsd)[None], dtype=torch.float32)
        with torch.no_grad():
        ...
        c = (c.mean(0).numpy() * self.ysd + self.ymu).astype(np.float32)
...
def main():
    ...
    ap.add_argument("--config", default="pi0_gate_aug"); ap.add_argument("--port", type=int, default=8814)
    a = ap.parse_args()
    ...
    pin = VlmFlowPin(policy, a.pin_u, a.head)
    print(f"[serve_gate_pin_vlmflow] ready on ws://127.0.0.1:{a.port}", flush=True)
