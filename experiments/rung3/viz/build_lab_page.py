...
COLS = [[96, 235, 160], [60, 190, 130], [30, 140, 100], [180, 140, 255], [90, 170, 240], [230, 120, 200]]
...
def main():
    ...
    groups, ticks, rows = [], [], []
    for k, (key, r) in enumerate(res["results"].items()):
        arm, _, sig = key.partition("@"); label = NAMES[arm] + (f" σ {sig}" if sig else "")
        ok, bad = [], []
        for f in sorted(glob.glob(f"{RUN}/traj_lab_{slug}_{key.replace('@', '_s')}_[0-9]*.npy")):
