"""Regenerated demo trajectories (regen1, 2026-08-23) over the scene clouds, next to the OLD
...
AND gate_clearance at plan time.
  ...
  python3 build_regen_page.py   (writes regen_demos.html next to this file)
"""
import glob
import os
import sys
...
KEPT = os.path.expanduser("~/code/falsify/runs/regen1/kept")
NPD = os.path.expanduser("~/code/falsify/runs/regen1/np")
...
for course, scene, dem, title, note in [
    ("through_left_gate", "left", range(100, 150), "Left",
     "New fan starts across the real 0.7 m spread, funnels through pre_gate, transits dead-center."),
    ("through_right_gate", "right", range(150, 200), "Right",
     "The fix that mattered: old returns skimmed 0.21 m past the EAST post (grey, direct diagonal "
     "to the goal); new returns (blue) take the return_east berth — >0.4 m from the post on both "
     "segments. 70/70 planned variants validated."),
    ...
    groups = [
        ...
        {"label": f"regen1 kept 50 (judge+clearance-clean)", "color": NEW, "trajs": new_kept(course)},
    ] + markers(scene)
    SECS.append((title, cloudviewer.viewer_html(scene, groups, note=note,
                                                elem_id=f"v_{course}", max_pts=40000)))

body = "".join(f"<h2>{t}</h2>\n<div class='vc'>{h}</div>\n" for t, h in SECS)
page = f"""<title>Regenerated Demo Fans</title>
<style>
...
<main>
<h1>Regenerated Demo Fans</h1>
<p class="sub">The regen1 dataset's kept trajectories (blue, 50 per task, every one
