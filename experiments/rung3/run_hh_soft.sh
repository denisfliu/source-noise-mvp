...
# pin's state->c prior; closed-loop eval pin (prior) vs scratch (unpinned). All -> htoh_soft.status.
set -uo pipefail
...
train() {  # exp init pinU|"" episodes  (assignment-from-expansion is not honored, so export/unset)
  ...
  CUDA_VISIBLE_DEVICES=$GPU XLA_PYTHON_CLIENT_MEM_FRACTION=0.9 WANDB_MODE=disabled \
    SNMVP_INIT_CKPT="$2" SNMVP_EPISODES="$4" SNMVP_PIN_NOISE=0.7 \
...
serve_eval() {  # exp prior|"" suite local out
  ...
  CUDA_VISIBLE_DEVICES=$GPU XLA_PYTHON_CLIENT_MEM_FRACTION=0.6 WANDB_MODE=disabled \
    $UV run serve_pca_pin.py --dir "$CKD/$exp/1999" --U "$U" ${prior:+--prior $prior} \
...
for g in 11 21; do
  ...
  # adapt both arms
  train "hhs_pin_t${g}"     "$CKD/snmvp_src_pin_vlm_soft/4999"       "$U" "$FS"; echo "ADAPT pin g=$g rc=$? $(date -u +%H:%M:%S)" >> "$ST"
  train "hhs_scratch_t${g}" "$CKD/snmvp_src_scratch_shared/4999" ""   "$FS"; echo "ADAPT scratch g=$g rc=$? $(date -u +%H:%M:%S)" >> "$ST"
  # pin prior on the same demos
  SNMVP_PIN_U=$U SNMVP_EPISODES=$FS SNMVP_PRIOR_OUT=$RD/prior_hhs_t${g}.npz SNMVP_NB=40 \
    CUDA_VISIBLE_DEVICES=$GPU "$HOME/code/openpi/.venv/bin/python" "$RD/fit_prior_hh.py" >> "$ST" 2>&1
  # closed-loop
  echo "EVAL pin g=$g" >> "$ST";     serve_eval "hhs_pin_t${g}"     "$RD/prior_hhs_t${g}.npz" "$su" "$LOCAL" "$RD/hhs_pin_t${g}.json"
  echo "EVAL scratch g=$g" >> "$ST"; serve_eval "hhs_scratch_t${g}" ""                        "$su" "$LOCAL" "$RD/hhs_scratch_t${g}.json"
done
