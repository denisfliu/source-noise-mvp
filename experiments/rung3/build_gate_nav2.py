"""Build local/gate_nav2: the regenerated training set (2026-08-23).
...
CFL 100-149 / CFR 150-199 / L 200-249 / R 250-299) so every episode-indexed tool keeps working:
  ...
  - SYNTH 100-299: the regen1 renders (fixed course family: CFR cross_west return + funnels +
...
gate_{synth,real}_eps2.json.

  ~/code/openpi/.venv/bin/python build_gate_nav2.py
"""
import csv
import glob
import io
...
REGEN = os.path.expanduser("~/code/falsify/runs/regen1/staging")
REPO = os.environ.get("REPO", "local/gate_nav2")
...
def main():
    ...
    # --- new synth from regen1 staging, fixed task order ---
    synth = []
    ...
    json.dump(synth, open(f"{RD}/gate_synth_eps2.json", "w"))
    json.dump(real, open(f"{RD}/gate_real_eps2.json", "w"))
    print(f"GATE_NAV2_DONE repo={REPO} real={len(real)} synth={len(synth)} out={out}")
