...
cd "$HOME/code/openpi"
ST=$RD/hhv_eval.status; echo "HHEVAL_START gpu=$GPU $(date -u +%H:%M:%S)" >> "$ST"
...
for g in 11 21; do
  ...
  serve_eval "$CKD/hhv_pin_t${g}/1999"     "$RD/prior_hhv_t${g}.npz" "$su" "$LOCAL" "$RD/hhv_pin_t${g}.json"
  serve_eval "$CKD/hhv_scratch_t${g}/1999" ""                        "$su" "$LOCAL" "$RD/hhv_scratch_t${g}.json"
done
