...
def _profile(n=H):
    """Cosine speed profile over n steps: per-step fractions summing to one, zero at both ends."""
    w = 1.0 - np.cos(np.linspace(0, 2 * math.pi, n + 2)[1:-1])
    return w / w.sum()
